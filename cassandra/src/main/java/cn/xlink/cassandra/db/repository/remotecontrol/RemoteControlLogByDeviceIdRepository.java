 ...
 */
public class RemoteControlLogByDeviceIdRepository extends SuperRemoteControlLogRepository {

    public RemoteControlLogByDeviceIdRepository(Session session) {
        super(session, CassandraConstants.T_REMOTE_CONTROL_LOG_DEVICE);
    }

    @Override
    protected RemoteControlLogEntity marshalRow(Row row) {
        RemoteControlLogEntity entity = new RemoteControlLogEntity();
        entity.setKey(row.getInt("device_id"));
