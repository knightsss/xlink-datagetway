package cn.xlink.cassandra.start.server;
...
import cn.xlink.cassandra.serialization.SerializationProvider;
import cn.xlink.cassandra.start.config.Configuration;
import cn.xlink.cassandra.start.relation.Relationships;
import cn.xlink.cassandra.start.routes.Routes;
...
public class Server {
    private static final String SERVICE_NAME = "data-cassandra";
    private static final Logger LOG = Logger.getLogger(SERVICE_NAME);

    private RestExpress restServer;
    ...
    public Server(Configuration config) {
        ...
        this.restServer = new RestExpress()
                .setName(SERVICE_NAME)
                .setBaseUrl(config.getBaseUrl())
                .setExecutorThreadCount(config.getExecutorThreadPoolSize())
                .addMessageObserver(new SimpleConsoleLogMessageObserver());

        Routes.init(restServer);
        Relationships.define(restServer);
        configurePlugins(config, restServer);
        mapExceptions(restServer);
    }

    public Server start() {
        if (!isStarted) {
            restServer.bind(config.getPort());
            isStarted = true;
        }
        return this;
    }
    ...
    public void awaitShutdown() {
        if (isStarted) restServer.awaitShutdown();
    }

    public void shutdown() {
        if (isStarted) restServer.shutdown();
    }
