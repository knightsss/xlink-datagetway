...
import cn.xlink.cassandra.service.DeviceOfflineService;
...
public class DeviceOfflineController extends BaseController<DeviceOfflineEntity> {
    public DeviceOfflineController(Set<RoleType> roles) {
        super(roles);
    }
    ...
    public Object get(Request request, Response response) throws XlinkException {
        ...
        DeviceOfflineService service = new DeviceOfflineService(scope);
        TwoTuple<Date, Date> betweenDate = funcGetBetweenDate(request);
        int limit = getLimit(request);
        int page = getPage(request);
        List<DeviceOfflineEntity> entities = readByPaging(service, limit, page, param, betweenDate);
        ...
        for (int i = 0, size = entities.size(); i < size; i++) {
            json = new JSONObject();
            DeviceOfflineEntity device = entities.get(i);
     ...
     * @param device
     */
    private void funGetReturnJson(JSONObject json, DeviceOfflineEntity device) {
        ...
        json.put("online_count", device.getOnlineCount());
