...
public class JdbcMetadataController
{
	...
	public JdbcMetadataController(Properties p,
	...
	public Object create(Request request, Response response) {
		JdbcMetadata entity = request.getBodyAs(JdbcMetadata.class, "Resource details not provided");
		...
		JdbcMetadata saved = jdbcService.create(entity);
	...
	{
		String id = request.getHeader(Constants.Url.JDBC_ID, "No resource ID supplied");

		try {
		    // check auth
			authCheck(request, response);

			JdbcMetadata entity = jdbcService.read(Identifiers.MONGOID.parse(id));
	...
	public void delete(Request request, Response response)
	{
		...
		try {
			authCheck(request, response);
			jdbcService.delete(Identifiers.MONGOID.parse(id));
		} catch (ItemNotFoundException e1) {
			response.setResponseCode(400);
			response.setException(e1);
		} catch (Exception e) {
