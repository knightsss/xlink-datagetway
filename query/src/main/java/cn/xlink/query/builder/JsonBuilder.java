...
import cn.xlink.query.domain.DruidJsonQueryBody;
...
import static cn.xlink.query.domain.Metric.METRIC_AVG;
import static cn.xlink.query.domain.Metric.METRIC_COUNT;
import static cn.xlink.query.domain.Metric.METRIC_SUM;
...
//                        }
                        switch (metric.getType().toLowerCase()) {
                            case METRIC_COUNT:
                                if (aggregationSet.add(METRIC_COUNT)) {
                            ...
                            case METRIC_AVG:
                                ...
                                if (postAggregationSet.add(METRIC_AVG + "_" + key)) {
                                    ...
                                    postAggregation.setName(METRIC_AVG + "_" + key);
                                    postAggregation.setFn("/");
                                    List<Aggregation> fields = new ArrayList<>();
                                    fields.add(new Aggregation(Aggregation.AGGREGATION_TYPE_FIELD_ACCESS, METRIC_SUM + "_" + key, METRIC_SUM + "_" + key));
                            ...
                            case METRIC_SUM:
                                if (aggregationSet.add(METRIC_SUM + "_" + key)) {
                                    String type;
                                    // TODO check field type decide aggregation type
                                    if (DataType.checkMetricType(body.getTypeMap().get(key))){
                                        type = Aggregation.AGGREGATION_TYPE_DOUBLE_SUM;
                                    } else {
                                        type = Aggregation.AGGREGATION_TYPE_LONG_SUM;
                                    }
                                    Aggregation aggregation = new Aggregation(type, METRIC_SUM + "_" + key, METRIC_SUM + "_" + key );
                                    aggregation.setTypeByFieldType(body.getTypeMap().get(key).druidDesc());
            ...
            if (!body.invalidSort()) {
                ...
                for (String sort : body.getSorts()) {
                    if (sort != null) {
                        ...
                        if (m.find()) {
                            ...
                            if (!DataType.checkMetricType(body.getTypeMap().get(sort))) {
                ...
                queryBody.setLimitSpec(
                        new Limit(
                                "default",
                                (body.invalidThreshold() ? DEFAULT_THRESHOLD : (int) body.getOptions().get("threshold")),
                                orderColumns));
            }
        }
