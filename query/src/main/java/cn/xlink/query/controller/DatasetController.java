...
import cn.xlink.metadata.datasetMetadata.DatasetMetadataFieldEntity;
...
//        filter.addCriteria("corpId",  FilterOperator.EQUALS, corpId);
        List<DatasetMetadata> list = this.getDatasetService().readAll(filter, range, order);
    ...
    public void fields(Request request, Response response) {
        ...
        if ((metadata = this.getDatasetService().findByEntity(dataset)) != null) {
            // 需要检查字段的来源
